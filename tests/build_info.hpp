...
inline void show_build_info()
{
    ...
    ss << "build info:\n"
       << "{\n";
    ss << tab << "os: " << (info.is_mac ? "mac" : "linux") << "\n";
    ss << tab << "is_clang: " << show_bool(info.is_clang) << "\n";
    ss << tab << "is_gcc: " << show_bool(info.is_gcc) << "\n";
    ss << tab << "have_openblas: " << show_bool(info.have_openblas) << "\n";
    if (info.have_openblas) {
        ss << tab << "openblas version: " << info.openblas_version << "\n";
    }
    ss << "}\n";
    printf("%s\n", ss.str().c_str());
}
...
defer_t _show_build_info([] { show_build_info(); });
